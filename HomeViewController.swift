...
class HomeViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    let servicosButton: UIButton = {
        ...
        button.setTitle("Estatisca", for: .normal)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if(dados[indexPath.row].status == "Vazio"){
            let detalheVC = SlotViewController()
            detalheVC.slot = dados[indexPath.row].id // Passar os dados para o próximo ViewController
            navigationController?.pushViewController(detalheVC, animated: true) // Navegar para a nova tela
        }else{
            let detalheVC = RetirarViewController()
